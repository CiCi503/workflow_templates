...
download_audio() {
    ...
    if [ -f "$TARGET_DIR/audio/wav2vec2_large_english_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/audio_encoders/wav2vec2_large_english_fp16.safetensors" --local-dir "$TARGET_DIR/audio" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/audio/whisper_large_v3_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HuMo_ComfyUI" "split_files/audio_encoders/whisper_large_v3_fp16.safetensors" --local-dir "$TARGET_DIR/audio" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
...
download_checkpoints() {
    ...
    if [ -f "$TARGET_DIR/checkpoints/512-inpainting-ema.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/stable_diffusion_2.1_repackaged" "512-inpainting-ema.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/DreamShaper_8_pruned.safetensors" ]; then
        ...
        huggingface-cli download "Lykon/DreamShaper" "DreamShaper_8_pruned.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/NetaYumev35_pretrained_all_in_one.safetensors" ]; then
        ...
        huggingface-cli download "duongve/NetaYume-Lumina-Image-2.0" "NetaYumev35_pretrained_all_in_one.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/ace_step_v1_3.5b.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/ACE-Step_ComfyUI_repackaged" "all_in_one/ace_step_v1_3.5b.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/hunyuan3d-dit-v2-mv-turbo_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/hunyuan3D_2.0_repackaged" "split_files/hunyuan3d-dit-v2-mv-turbo_fp16.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/hunyuan3d-dit-v2-mv_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/hunyuan3D_2.0_repackaged" "split_files/hunyuan3d-dit-v2-mv_fp16.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/hunyuan3d-dit-v2_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/hunyuan3D_2.0_repackaged" "split_files/hunyuan3d-dit-v2_fp16.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/hunyuan_3d_v2.1.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/hunyuan3D_2.1_repackaged" "hunyuan_3d_v2.1.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/ltx-video-2b-v0.9.5.safetensors" ]; then
        ...
        huggingface-cli download "Lightricks/LTX-Video" "ltx-video-2b-v0.9.5.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/sd3.5_large_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/stable-diffusion-3.5-fp8" "sd3.5_large_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/sd_xl_base_1.0.safetensors" ]; then
        ...
        huggingface-cli download "stabilityai/stable-diffusion-xl-base-1.0" "sd_xl_base_1.0.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/stable-audio-open-1.0.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/stable-audio-open-1.0_repackaged" "stable-audio-open-1.0.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/checkpoints/v1-5-pruned-emaonly-fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/stable-diffusion-v1-5-archive" "v1-5-pruned-emaonly-fp16.safetensors" --local-dir "$TARGET_DIR/checkpoints" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
...
download_clip() {
    ...
    if [ -f "$TARGET_DIR/clip/byt5_small_glyphxl_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HunyuanVideo_1.5_repackaged" "split_files/text_encoders/byt5_small_glyphxl_fp16.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/clip_g_hidream.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HiDream-I1_ComfyUI" "split_files/text_encoders/clip_g_hidream.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/clip_l.safetensors" ]; then
        ...
        huggingface-cli download "comfyanonymous/flux_text_encoders" "clip_l.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/clip_l_hidream.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HiDream-I1_ComfyUI" "split_files/text_encoders/clip_l_hidream.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/llama_3.1_8b_instruct_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HiDream-I1_ComfyUI" "split_files/text_encoders/llama_3.1_8b_instruct_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/mistral_3_small_flux2_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux2-dev" "split_files/text_encoders/mistral_3_small_flux2_bf16.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/mistral_3_small_flux2_fp8.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux2-dev" "split_files/text_encoders/mistral_3_small_flux2_fp8.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/qwen_2.5_vl_7b_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HunyuanVideo_1.5_repackaged" "split_files/text_encoders/qwen_2.5_vl_7b_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/qwen_2.5_vl_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Omnigen2_ComfyUI_repackaged" "split_files/text_encoders/qwen_2.5_vl_fp16.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/qwen_3_4b.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/z_image_turbo" "split_files/text_encoders/qwen_3_4b.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/t5-base.safetensors" ]; then
        ...
        huggingface-cli download "ComfyUI-Wiki/t5-base" "t5-base.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/t5xxl_fp16.safetensors" ]; then
        ...
        huggingface-cli download "comfyanonymous/flux_text_encoders" "t5xxl_fp16.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/t5xxl_fp8_e4m3fn_scaled.safetensors" ]; then
        ...
        huggingface-cli download "comfyanonymous/flux_text_encoders" "t5xxl_fp8_e4m3fn_scaled.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/umt5_xxl_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/text_encoders/umt5_xxl_fp16.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip/umt5_xxl_fp8_e4m3fn_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/text_encoders/umt5_xxl_fp8_e4m3fn_scaled.safetensors" --local-dir "$TARGET_DIR/clip" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
...
download_clip_vision() {
    ...
    if [ -f "$TARGET_DIR/clip_vision/clip_vision_g.safetensors" ]; then
        ...
        huggingface-cli download "comfyanonymous/clip_vision_g" "clip_vision_g.safetensors" --local-dir "$TARGET_DIR/clip_vision" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip_vision/clip_vision_h.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/clip_vision/clip_vision_h.safetensors" --local-dir "$TARGET_DIR/clip_vision" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/clip_vision/sigclip_vision_patch14_384.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/sigclip_vision_384" "sigclip_vision_patch14_384.safetensors" --local-dir "$TARGET_DIR/clip_vision" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
...
download_style_models() {
    ...
    if [ -f "$TARGET_DIR/style_models/flux1-redux-dev.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Flux1-Redux-Dev" "flux1-redux-dev.safetensors" --local-dir "$TARGET_DIR/style_models" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
...
download_unet() {
    ...
    if [ -f "$TARGET_DIR/unet/Chroma1-HD-fp8_scaled_rev2.safetensors" ]; then
        ...
        huggingface-cli download "silveroxides/Chroma1-HD-fp8-scaled" "Chroma1-HD-fp8_scaled_rev2.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/Wan21-WanMove_fp8_scaled_e4m3fn_KJ.safetensors" ]; then
        ...
        huggingface-cli download "Kijai/WanVideo_comfy_fp8_scaled" "WanMove/Wan21-WanMove_fp8_scaled_e4m3fn_KJ.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/Wan2_1-I2V-ATI-14B_fp8_e4m3fn.safetensors" ]; then
        ...
        huggingface-cli download "Kijai/WanVideo_comfy" "Wan2_1-I2V-ATI-14B_fp8_e4m3fn.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/Wan2_2-Animate-14B_fp8_e4m3fn_scaled_KJ.safetensors" ]; then
        ...
        huggingface-cli download "Kijai/WanVideo_comfy_fp8_scaled" "Wan22Animate/Wan2_2-Animate-14B_fp8_e4m3fn_scaled_KJ.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/flux1-canny-dev.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux1-dev" "split_files/diffusion_models/flux1-canny-dev.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/flux1-dev-fp8.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux1-dev" "flux1-dev-fp8.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/flux1-dev.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux1-dev" "flux1-dev.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/flux1-fill-dev.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux1-dev" "split_files/diffusion_models/flux1-fill-dev.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/flux1-schnell.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux1-schnell" "flux1-schnell.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/flux2_dev_fp8mixed.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux2-dev" "split_files/diffusion_models/flux2_dev_fp8mixed.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/hidream_e1_full_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HiDream-I1_ComfyUI" "split_files/diffusion_models/hidream_e1_full_bf16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/hidream_i1_fast_fp8.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HiDream-I1_ComfyUI" "split_files/diffusion_models/hidream_i1_fast_fp8.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/hidream_i1_full_fp8.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HiDream-I1_ComfyUI" "split_files/diffusion_models/hidream_i1_full_fp8.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/humo_17B_fp8_e4m3fn.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HuMo_ComfyUI" "split_files/diffusion_models/humo_17B_fp8_e4m3fn.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/hunyuanvideo1.5_1080p_sr_distilled_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HunyuanVideo_1.5_repackaged" "split_files/diffusion_models/hunyuanvideo1.5_1080p_sr_distilled_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/hunyuanvideo1.5_720p_i2v_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HunyuanVideo_1.5_repackaged" "split_files/diffusion_models/hunyuanvideo1.5_720p_i2v_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/hunyuanvideo1.5_720p_t2v_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HunyuanVideo_1.5_repackaged" "split_files/diffusion_models/hunyuanvideo1.5_720p_t2v_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/omnigen2_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Omnigen2_ComfyUI_repackaged" "split_files/diffusion_models/omnigen2_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/qwen_image_edit_2509_fp8_e4m3fn.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Qwen-Image-Edit_ComfyUI" "split_files/diffusion_models/qwen_image_edit_2509_fp8_e4m3fn.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/qwen_image_fp8_e4m3fn.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Qwen-Image_ComfyUI" "split_files/diffusion_models/qwen_image_fp8_e4m3fn.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_flf2v_720p_14B_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_flf2v_720p_14B_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_fun_camera_v1.1_1.3B_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_fun_camera_v1.1_1.3B_bf16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_fun_camera_v1.1_14B_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_fun_camera_v1.1_14B_bf16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_fun_control_1.3B_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_fun_control_1.3B_bf16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_i2v_480p_14B_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_i2v_480p_14B_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_t2v_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_t2v_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_vace_1.3B_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_vace_1.3B_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.1_vace_14B_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/diffusion_models/wan2.1_vace_14B_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_camera_high_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_camera_high_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_camera_low_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_camera_low_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_control_5B_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_control_5B_bf16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_control_high_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_control_high_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_control_low_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_control_low_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_inpaint_5B_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_inpaint_5B_bf16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_inpaint_high_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_inpaint_high_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_fun_inpaint_low_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_fun_inpaint_low_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_i2v_high_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_i2v_high_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_i2v_low_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_i2v_low_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_s2v_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_s2v_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_t2v_high_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_t2v_high_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_t2v_low_noise_14B_fp8_scaled.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_t2v_low_noise_14B_fp8_scaled.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/wan2.2_ti2v_5B_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/diffusion_models/wan2.2_ti2v_5B_fp16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unet/z_image_turbo_bf16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/z_image_turbo" "split_files/diffusion_models/z_image_turbo_bf16.safetensors" --local-dir "$TARGET_DIR/unet" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
...
download_unknown() {
    ...
    if [ -f "$TARGET_DIR/unknown/Qwen-Image-Edit-2509-Relight.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Qwen-Image-Edit_ComfyUI" "split_files/loras/Qwen-Image-Edit-2509-Relight.safetensors" --local-dir "$TARGET_DIR/unknown" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/unknown/lotus-depth-d-v1-1.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/lotus" "lotus-depth-d-v1-1.safetensors" --local-dir "$TARGET_DIR/unknown" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
...
download_vae() {
    ...
    if [ -f "$TARGET_DIR/vae/ae.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/z_image_turbo" "split_files/vae/ae.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/flux2-vae.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/flux2-dev" "split_files/vae/flux2-vae.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/hunyuanvideo15_vae_fp16.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/HunyuanVideo_1.5_repackaged" "split_files/vae/hunyuanvideo15_vae_fp16.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/qwen_image_vae.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Qwen-Image_ComfyUI" "split_files/vae/qwen_image_vae.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/vae-ft-mse-840000-ema-pruned.safetensors" ]; then
        ...
        huggingface-cli download "stabilityai/sd-vae-ft-mse-original" "vae-ft-mse-840000-ema-pruned.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/wan2.2_vae.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.2_ComfyUI_Repackaged" "split_files/vae/wan2.2_vae.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/wan_2.1_vae.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/vae/wan_2.1_vae.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/wan_alpha_2.1_vae_alpha_channel.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/vae/wan_alpha_2.1_vae_alpha_channel.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/vae/wan_alpha_2.1_vae_rgb_channel.safetensors" ]; then
        ...
        huggingface-cli download "Comfy-Org/Wan_2.1_ComfyUI_repackaged" "split_files/vae/wan_alpha_2.1_vae_rgb_channel.safetensors" --local-dir "$TARGET_DIR/vae" --local-dir-use-symlinks False --revision main
        if [ $? -eq 0 ]; then
